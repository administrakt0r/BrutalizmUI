import * as React from "react"

import { cn } from "@/lib/utils"

export type Star25Props = React.ComponentPropsWithoutRef<"svg"> & {
  color?: string
  size?: number
  stroke?: string
  pathClassName?: string
  strokeWidth?: number
}

/**
 * ⚡ Bolt: Star25 component optimized with React.memo and React.forwardRef.
 * @preview ![img](data:image/png;base64,)
 */
const Star25 = React.memo(
  React.forwardRef<SVGSVGElement, Star25Props>(
    (
      {
        color,
        size,
        stroke,
        strokeWidth,
        pathClassName,
        width,
        height,
        className,
        ...props
      },
      ref,
    ) => {
      return (
        <svg
          ref={ref}
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 200 200"
          width={size ?? width}
          height={size ?? height}
          data-slot="star-s25"
          className={cn(className)}
          {...props}
        >
          <path
        fill={color ?? "currentColor"}
        stroke={stroke}
        strokeWidth={strokeWidth}
        className={pathClassName}
        d="M118.05 118.05c102.6 102.6-138.7 102.6-36.1 0-102.6 102.6-102.6-138.7 0-36.1-102.6-102.6 138.7-102.6 36.1 0 102.6-102.6 102.6 138.7 0 36.1"
      />
        </svg>
      )
    },
  ),
)

Star25.displayName = "Star25"

export default Star25
