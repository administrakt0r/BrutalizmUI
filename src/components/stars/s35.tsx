import * as React from "react"

import { cn } from "@/lib/utils"

export type Star35Props = React.ComponentPropsWithoutRef<"svg"> & {
  color?: string
  size?: number
  stroke?: string
  pathClassName?: string
  strokeWidth?: number
}

/**
 * ⚡ Bolt: Star35 component optimized with React.memo and React.forwardRef.
 * @preview ![img](data:image/png;base64,)
 */
const Star35 = React.memo(
  React.forwardRef<SVGSVGElement, Star35Props>(
    (
      {
        color,
        size,
        stroke,
        strokeWidth,
        pathClassName,
        width,
        height,
        className,
        ...props
      },
      ref,
    ) => {
      return (
        <svg
          ref={ref}
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 200 200"
          width={size ?? width}
          height={size ?? height}
          data-slot="star-s35"
          className={cn(className)}
          {...props}
        >
          <path
        fill={color ?? "currentColor"}
        stroke={stroke}
        strokeWidth={strokeWidth}
        className={pathClassName}
        d="M60.575 5.095C81.665 28.37 118.24 28.37 139.235 5c-1.615 31.35 24.32 57.19 55.67 55.575-23.275 21.09-23.275 57.665.095 78.66-31.35-1.615-57.19 24.32-55.575 55.67-21.09-23.275-57.665-23.275-78.66.095 1.615-31.35-24.32-57.19-55.67-55.575C28.37 118.335 28.37 81.76 5 60.765c31.35 1.52 57.19-24.32 55.575-55.67"
      />
        </svg>
      )
    },
  ),
)

Star35.displayName = "Star35"

export default Star35
